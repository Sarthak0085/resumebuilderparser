...
export type GeneralSettings = Exclude<
  keyof Settings,
  'formToHeading' | 'formToShow' | 'formsOrder' | 'showBulletPoints'
>;
...
export const settingsSlice = createSlice({
  ...
  reducers: {
    changeSettings: (draft, action: PayloadAction<{ field: GeneralSettings; value: string }>) => {
    ...
    changeFormOrder: (draft, action: PayloadAction<{ form: ShowForm; type: 'up' | 'down' }>) => {
      ...
      const swapFormOrder = (idx1: number, idx2: number) => {
        [draft.formsOrder[idx1], draft.formsOrder[idx2]] = [
          draft.formsOrder[idx2],
          draft.formsOrder[idx1],
        ];
      };
