...
export const initialWorkExperiences: ResumeWorkExperience = {
  jobTitle: '',
  company: '',
  date: '',
  descriptions: [],
    ...
    addSectionInForm: (draft, action: PayloadAction<{ form: ShowForm }>) => {
      ...
      switch (form) {
        case 'workExperiences': {
          draft.workExperiences.push(structuredClone(initialWorkExperiences));
          return draft;
        }
        case 'educations': {
        ...
        case 'projects': {
          draft.projects.push(structuredClone(initialProjects));
          return draft;
        }
      }
    },
