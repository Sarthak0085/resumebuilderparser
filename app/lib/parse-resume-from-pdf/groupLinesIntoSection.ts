...
const isSectionTitle = (line: Line, lineNumber: number) => {
  ...
  if (isFirstTwoLine || hasMoreThanOneItemInLine || hasNoItemInLine) {
    return false;
  }
  ...
  const startWithCapitalLetter = /[A-Z]/.test(text.slice(0, 1));
...
export const isBold = (item: TextItem) => isTextItemBold(item.font);
